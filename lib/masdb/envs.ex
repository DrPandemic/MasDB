defmodule Masdb.Envs do
  ...
  node_to_join Set the node that this node need to contact to join a cluster. It should be using the form name@hostname.
  ...
  def get_config(config \\ Application.get_all_env(:masdb)) do
    ...
    {
      Keyword.get(config, :save_file) || @default_file,
      "#{node_name}@#{hostname}",
      Keyword.get(config, :node_to_join)
    }
  end
